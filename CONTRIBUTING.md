...
#### 1. Separate Room from Fists Tool
Extract room management (create/join/disconnect/reconnect) from the fists combat tool into a standalone room module. Currently the room UI and fists UI are coupled in `multiplayer.html` and `room.rs`. Separating them makes the room a reusable foundation for new multiplayer tools (shared turn timer, future game modes). The multiplayer button would show room status, once connected new multiplayer tools will appear in the toolbar.
...
#### 3. QR Room Join
Embed the room code in a QR code so scanning joins the room directly. This connects two existing features (QR scanner + multiplayer) with minimal new code. The flow: Player A creates a room → room code appears as both text and a QR. Player B scans the QR → auto-joins the room. The QR URL format could be `kpks.us/join?code=ABCD#room=myroom` with a redirect that passes the code to the multiplayer module. The QR interface will need a switch camera button (for mobile users). Adding the function should be trival.

#### 4. Infinite Scroll with Content-Visibility
Replace the sentinel-chain pagination on the index page with a bulk-load + `content-visibility: auto` + `x-intersect` fade approach. Load 12 cards for fast initial paint, then after 1 second bulk-load all remaining cards (invisible, `opacity-0`). Use `x-intersect` to fade cards in/out as they enter/leave the viewport. `content-visibility: auto` tells the browser to skip layout/paint for off-screen cards. This eliminates scroll jank from overlapping sentinel-triggered HTMX requests during fast scrolling. Also explore implementing full virtual (DOM swaps in a loop) scroll for future proofing (goal = many more cards)
...
#### 6. Decentralized Identity & Authentication (Yrs Foundation)
...
**Phase 6b: Local Keypair Identity**
...
**Phase 6c: yrs-Based State Containers**
...
#### 7. Deck Builder / Hand Management
...
#### 9. Affinity & Loyalty Tracking *(post-Yrs)*
...
#### 8. Card Trading *(post-launch)*
Exchange cards between connected players via the WebRTC data channel. Requires the game to be publicly available with a real player base to validate the mechanic. Architecture is straightforward (new data channel message type + room state extension) but the game design implications need playtesting.

#### 9. Spectator Mode *(post-launch)*
