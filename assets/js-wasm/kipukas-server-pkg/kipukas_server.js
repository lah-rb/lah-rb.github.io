 ...
 * @returns {Uint8Array}
 */
export function decode_jxl(bytes, max_dim) {
    const ptr0 = passArray8ToWasm0(bytes, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.decode_jxl(ptr0, len0, max_dim);
    if (ret[3]) {
 ...
 * @returns {string}
 */
export function handle_request(method, path, query, body) {
    let deferred5_0;
    let deferred5_1;
    try {
        ...
        const ret = wasm.handle_request(ptr0, len0, ptr1, len1, ptr2, len2, ptr3, len3);
        deferred5_0 = ret[0];
        deferred5_1 = ret[1];
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('kipukas_server_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
